    ...
    public:
        ...
        struct boss_unsokAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                // Opening doors
                if (GameObject* entranceDoor = pInstance->instance->GetGameObject(pInstance->GetData64(GOB_SANCTUM_DOOR_ENTRANCE)))
                    entranceDoor->SetGoState(GO_STATE_ACTIVE);
                if (GameObject* exitDoor = pInstance->instance->GetGameObject(pInstance->GetData64(GOB_HEARTOFFEAR_DOOR_ENTRANCE)))
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FLING:
                        {
                            if (Unit* target = ObjectAccessor::FindUnit(grabTargetGuid))
        ...
        class spell_fling_SpellScript : public SpellScript
        {
            ...
            void Handle(SpellEffIndex /*effIndex*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    uint64 targetGuid = CAST_AI(mob_amber_monstruosity::mob_amber_monstruosityAI, caster->GetAI())->grabTargetGuid;
                    if (Unit* target = ObjectAccessor::FindUnit(targetGuid))
                    {
