...
/// Phase handler
class playerScript_enter_wandering_north : public PlayerScript
{
public:
	playerScript_enter_wandering_north() : PlayerScript("playerScript_enter_wandering_north") { }
	...
	void UpdatePhaseMask(Player* p_Player)
	{
		...
		if (p_Player->GetQuestStatus(30042 || 30039 || 30041 || 30044 || 30004 || 30043 || 30040 || 30045) == QUEST_STATUS_COMPLETE)
		{
			l_PhaseMask == 5; // To avoid weird bugs we ensure the phases are correct at the starting of the questline
		}

		if (p_Player->GetQuestStatus(29524) == QUEST_STATUS_COMPLETE)
		{
			l_PhaseMask &= ~4;
			l_PhaseMask |= 8;
		}

		if (p_Player->GetQuestStatus(29409) == QUEST_STATUS_COMPLETE)
		{
			l_PhaseMask &= ~4;
			l_PhaseMask &= ~8;
			l_PhaseMask |= 2;
		...
		if (p_Player->GetQuestStatus(29523) == QUEST_STATUS_COMPLETE)
		{
			l_PhaseMask &= ~4;
			l_PhaseMask &= ~8;
			l_PhaseMask &= ~2;
...
{
	new playerScript_enter_wandering_north();
