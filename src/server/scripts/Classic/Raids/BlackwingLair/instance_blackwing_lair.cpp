	...
	struct instance_blackwing_lair_InstanceMapScript : public InstanceScript
	{
		...
		void OnGameObjectCreate(GameObject* go)
		{
			...
			case 175185: // Door
				...
				HandleGameObject(0, GetBossState(DATA_VAELASTRAZ_THE_CORRUPT) == DONE, go);
				break;
			case 180424: // Door
				BroodlordDoorGUID = go->GetGUID();
				HandleGameObject(0, GetBossState(DATA_BROODLORD_LASHLAYER) == DONE, go);
				break;
			case 185483: // Door
				ChrommagusDoorGUID = go->GetGUID();
				HandleGameObject(0, GetBossState(DATA_FIREMAW) == DONE && GetBossState(DATA_EBONROC) == DONE && GetBossState(DATA_FLAMEGOR) == DONE, go);
				break;
			case 181125: // Door
				NefarianDoorGUID = go->GetGUID();
				HandleGameObject(0, GetBossState(DATA_CHROMAGGUS) == DONE, go);
				break;
			}
		}
		...
		bool SetBossState(uint32 type, EncounterState state)
		{
			...
			case DATA_EBONROC:
			case DATA_FLAMEGOR:
				HandleGameObject(ChrommagusDoorGUID, GetBossState(DATA_FIREMAW) == DONE && GetBossState(DATA_EBONROC) == DONE && GetBossState(DATA_FLAMEGOR) == DONE);
				break;
