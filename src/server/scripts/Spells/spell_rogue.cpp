...
/// Last Update 6.2.3
/// Death from Above Return - 178236
...
public:
	...
	class spell_rog_death_from_above_return_SpellScript : public SpellScript
	{
		...
		void HandleAfterCast()
		{
			...
			Player* p_Player = l_Caster->ToPlayer();
			Unit* l_Target = l_Caster->ToPlayer()->GetSelectedUnit();
			if (l_Target && l_Caster->ToPlayer()->getClass() == CLASS_ROGUE)
			{
		...
		void Register()
		{
			AfterCast += SpellCastFn(spell_rog_death_from_above_return_SpellScript::HandleAfterCast);
		}
	};
    ...
    public:
        ...
        class spell_rog_death_from_above_SpellScript : public SpellScript
        {
            ...
            void HandleAfterCast()
            {
				...
				l_Caster->CastSpell(l_Caster, eSpells::DeathFromAboveLeap, true);
            }
        ...
        class spell_rog_death_from_above_AuraScript : public AuraScript
        {
            ...
            enum eSpells
            {
				DeathFromAboveJump1  = 178153,
                DeathFromAboveJump2  = 178236,
                SpellRogueEnvenom    = 32645,
                SpellRogueEviscerate = 2098
            };
            ...
            void OnTick(AuraEffect const* p_AurEff)
            {
                ...
                if (Player* l_Player = GetCaster()->ToPlayer())
                {
                    if (Unit* l_Target = GetTarget())
                    {
						l_Player->CastSpell(l_Target, eSpells::DeathFromAboveJump1, true);
                        l_Player->CastSpell(l_Target, eSpells::DeathFromAboveJump2, true);
    ...
    public:
        ...
        class spell_rog_blade_flurry_AuraScript : public AuraScript
        {
            ...
            void OnProc(AuraEffect const* /*p_AurEff*/, ProcEventInfo& p_EventInfo)
            {
                ...
                if (Unit* l_Caster = GetCaster())
                {
					DamageInfo const* m_info = p_EventInfo.GetDamageInfo();
					SpellInfo const* procSpell = p_EventInfo.GetDamageInfo()->GetSpellInfo();
					int32 damage = m_info->GetDamage();
					if (!damage || m_info->GetDamageType() == DOT)
						return;

                    if (!damage || p_EventInfo.GetDamageInfo()->GetDamageType() == DOT)
                        return;

                    if (procSpell && !procSpell->CanTriggerBladeFlurry())
                        return;

                    if (Unit* l_Target = p_EventInfo.GetDamageInfo()->GetVictim())
                        l_Caster->CastCustomSpell(l_Target, ROGUE_SPELL_BLADE_FLURRY_DAMAGE, &damage, NULL, NULL, true);
                }
            }
