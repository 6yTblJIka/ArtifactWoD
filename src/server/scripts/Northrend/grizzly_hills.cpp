...
class npc_tunnels_bunny1 : public CreatureScript
{
public:
	npc_tunnels_bunny1() : CreatureScript("npc_tunnels_bunny1") { }

	struct npc_tunnels_bunny1AI : public ScriptedAI
	{
		npc_tunnels_bunny1AI(Creature* creature) : ScriptedAI(creature) {}
		...
		void UpdateAI(const uint32 /*uiDiff*/)
		{
			...
			{
				if ((*itr)->HasQuest(12003))
					(*itr)->KilledMonsterCredit(26468, NULL);
			}
		}
	};
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_tunnels_bunny1AI(creature);
	}
};

class npc_tunnels_bunny2 : public CreatureScript
{
public:
	npc_tunnels_bunny2() : CreatureScript("npc_tunnels_bunny2") { }

	struct npc_tunnels_bunny2AI : public ScriptedAI
	{
		npc_tunnels_bunny2AI(Creature* creature) : ScriptedAI(creature) {}
		...
		void UpdateAI(const uint32 /*uiDiff*/)
		{
			...
			{
				if ((*itr)->HasQuest(12003))
					(*itr)->KilledMonsterCredit(26469, NULL);
			}
		}
	};
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_tunnels_bunny2AI(creature);
	}
};

class npc_tunnels_bunny3 : public CreatureScript
{
public:
	npc_tunnels_bunny3() : CreatureScript("npc_tunnels_bunny3") { }

	struct npc_tunnels_bunny3AI : public ScriptedAI
	{
		npc_tunnels_bunny3AI(Creature* creature) : ScriptedAI(creature) {}
		...
		void UpdateAI(const uint32 /*uiDiff*/)
		{
			...
			{
				if ((*itr)->HasQuest(12003))
					(*itr)->KilledMonsterCredit(26470, NULL);
			}
		}
	};
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_tunnels_bunny3AI(creature);
	}
};
...
#define NPC_EARTHERN_WARRIOR    26282

class npc_runic_battle_golem : public CreatureScript
{
public:
	npc_runic_battle_golem() : CreatureScript("npc_runic_battle_golem") { }

	struct npc_runic_battle_golemAI : public ScriptedAI
	{
		npc_runic_battle_golemAI(Creature* creature) : ScriptedAI(creature) {}

		uint32 waitTime;
		Creature* warriorTarget;

		void Reset() override
		{
			warriorTarget = nullptr;
		...
		void UpdateAI(uint32 diff) override
		{
			...
			if (!warriorTarget)
			{
				// Need to define earthernWarrior
				if (Creature* earthernWarrior = ObjectAccessor::GetCreature(*me, warriorTarget->GetGUID()))
				{
					if (earthernWarrior->isAlive())
					{
						if (me->getVictim() != earthernWarrior)
						{
							me->getThreatManager().addThreat(earthernWarrior, 1000000.0f);
							earthernWarrior->getThreatManager().addThreat(me, 1000000.0f);
							me->Attack(earthernWarrior, true);
						}
					}
					else
					{
						earthernWarrior->DespawnOrUnsummon();
						warriorTarget = nullptr;
					}
				}
			}
			...
			{
				// Need to define warriorPos
				Position warriorPos;
				me->GetPosition(&warriorPos);
				GetPositionWithDistInFront(me, 5.0f, warriorPos);

				float z = me->GetMap()->GetHeight(me->GetPhaseMask(), warriorPos.GetPositionX(), warriorPos.GetPositionY(), warriorPos.GetPositionZ());
				warriorPos.m_positionZ = z;

				// Need to make multiple earthern warriors attack Runic Battle Golem
				if (Creature* earthernWarrior = me->SummonCreature(NPC_EARTHERN_WARRIOR, warriorPos))
				{
					me->getThreatManager().addThreat(earthernWarrior, 1000000.0f);
					earthernWarrior->getThreatManager().addThreat(me, 1000000.0f);
					AttackStart(earthernWarrior);
					earthernWarrior->SetFacingToObject(me);
					warriorTarget = earthernWarrior;
				}
			}
		}
	};
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_runic_battle_golemAI(creature);
	}
};
...
void AddSC_grizzly_hills()
{
	...
	new npc_tunnels_bunny1();
	new npc_tunnels_bunny2();
	new npc_tunnels_bunny3();
	new npc_runic_battle_golem();
}
#endif
