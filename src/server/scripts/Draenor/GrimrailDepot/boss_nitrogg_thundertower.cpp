    ...
    struct boss_nitrogg_thundertowerAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (!m_First)
            {
                ...
                me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_DISABLE_MOVE | eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
            }
            else if (m_Phase > 2)
                m_Phase = 3;
        }
        ...
        void JustReachedHome() override
        {
            ...
            if (m_Instance != nullptr)
            {
                if (GameObject* l_AssaultFlaps = m_Instance->instance->GetGameObject(m_Instance->GetData64(GrimrailDepotData::DataAssaultFlaps)))
                {
                ...
                if (GameObject* l_InvisibleDoor = m_Instance->instance->GetGameObject(m_Instance->GetData64(GrimrailDepotData::DataInvisibleDoor)))
                {
            ...
            if (!l_TurretsList.empty())
            {
                for (Creature* l_Itr : l_TurretsList)
                {
                    ...
                    l_Itr->Respawn(true);
                    l_Itr->NearTeleportTo(l_Itr->GetHomePosition().m_positionX, l_Itr->GetHomePosition().m_positionY, l_Itr->GetHomePosition().m_positionZ, l_Itr->GetHomePosition().m_orientation);
                }
            }
        }
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            ...
            case eNitroggThundertowerMovments::MovementPointThreeRemoveFlags:
                ...
                me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC );
                break;
            default:
                break;
            }
        ...
        {
            if (Player* l_Player = me->FindNearestPlayer(18.0f, true))
            {
            ...
            if (m_Phase == 3 && me->GetHealthPct() < 80 && events.ExecuteEvent() == eNitroggThundertowerEvents::EventOpenAssaultFlaps)
            {
                ...
                if (m_Instance != nullptr)
                {
                    if (Creature* l_NitroggTurret = m_Instance->instance->GetCreature(m_Instance->GetData64(GrimrailDepotData::DataNitroggTurret)))
                    {
                        events.Reset();
                        OpenAssaultFlaps();
                        ...
                        if (Player* l_NearestPlayer = l_NitroggTurret->FindNearestPlayer(100.0f, true))
                        {
            ...
            case eNitroggThundertowerEvents::EventReinforceBoomers:
                for (uint8 l_I = 0; l_I < 3; l_I++)
                {
                    ...
                    {
                        float l_X = (l_Creature->m_positionX + 15) * cos(l_Creature->m_orientation);
                        float l_Y = (l_Creature->m_positionY + 15) * sin(l_Creature->m_orientation);

                        l_Creature->GetMotionMaster()->MoveJump(l_X, l_Y, g_TrainFloorPositionZ);
                    }
                }
...
class grimrail_depot_nitrogg_thundertower_mob_iron_turret : public CreatureScript
{
    ...
    struct grimrail_depot_nitrogg_thundertower_mob_iron_turretAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_DISABLE_MOVE | eUnitFlags::UNIT_FLAG_NOT_SELECTABLE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
        }
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            case eIronTowerEvents::EventSuppressiveFire:
                ...
                me->MonsterSay("suppressive fire", LANG_UNIVERSAL, me->GetGUID());
            ...
            case eIronTowerEvents::EventSlagBlast:
                ...
                me->MonsterSay("slag blast", LANG_UNIVERSAL, me->GetGUID());
            ...
            case eGromkarboomerEvents::EventBlackrockMortarBombs:
            {
                ...
                if (Vehicle* l_Vehicle = me->GetVehicleKit())
                {
                    if (Unit* l_Passenger = l_Vehicle->GetPassenger(0))
                    {
                        ...
                        {
                            m_Modifier += 2;
                            float l_X = (me->m_positionX + 2 + m_Modifier) * cos(me->m_orientation);
                            float l_Y = (me->m_positionY + 2 + m_Modifier) * sin(me->m_orientation);
...
class grimrail_depot_nitrogg_thundertower_mob_assault_turret : public CreatureScript
{
    ...
    struct grimrail_depot_nitrogg_thundertower_mob_assault_turretAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            events.Reset();
            me->SetReactState(ReactStates::REACT_PASSIVE);
            me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_DISABLE_MOVE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
        }
            ...
            if (Unit* l_Caster = GetCaster())
            {
                if (Unit* l_Target = GetTarget())
                {
                    if (l_Target->IsWithinLOSInMap(l_Caster))
                    {
                        ...
                        l_Caster->GetFirstCollisionPosition(l_Pos, l_Dist, l_Angle);
                        ...
                        if (Creature* l_NearestTrigger = l_Caster->SummonCreature(eSuppressiveFireCreatures::CreatureIronTurretNitroggSuppressiveFire, l_Position, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 1 * TimeConstants::IN_MILLISECONDS))
                        {
                            ...
                            l_NearestTrigger->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_DISABLE_MOVE);
                            l_Caster->SendPlaySpellVisual(eNitroggThundertowerSpellVisuals::SpellFireSuppressionVisualId, l_NearestTrigger, 50.0f, l_Caster->GetOrientation(), Position());
                            l_Caster->CastSpell(l_NearestTrigger, eSuppressiveFireSpells::SpellSuppressiveFireTriggerMissile, true);
                        }
                    }
                    else
                    {
                        l_Caster->SendPlaySpellVisual(eNitroggThundertowerSpellVisuals::SpellFireSuppressionVisualId, l_Target, 50.0f, l_Caster->GetOrientation(), Position());
...
class grimrail_depot_nitrogg_thundertower_slag_blast : public SpellScriptLoader
{
    ...
    class grimrail_depot_nitrogg_thundertower_slag_blast_SpellScript : public SpellScript
    {
        ...
        void OnSpellHit(SpellEffIndex)
        {
            ...
            l_Caster->CastSpell(l_Dest, eSlagBlastSpells::SpellSlagBlastAreatrigger, true);
        }
...
class grimrail_depot_nitrogg_thundertower_at_slagblast : public AreaTriggerEntityScript
{
    ...
    void OnCreate(AreaTrigger* p_AreaTrigger) override
    {
        ...
        p_AreaTrigger->GetCreatureListWithEntryInGrid(l_ListCreatures, eSlagblastCreatures::CreatureSlagblast, 200.0f);
    ...
    grimrail_depot_nitrogg_thundertower_at_slagblast* GetAI() const override
    {
        return new grimrail_depot_nitrogg_thundertower_at_slagblast();
    }
};
