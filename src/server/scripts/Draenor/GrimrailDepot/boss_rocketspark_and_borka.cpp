...
public:
    ...
    bool Execute(uint64 /*p_CurrTime*/, uint32 /*p_Diff*/)
    {
        if (m_Obj)
        {
            ...
            {
                if (Creature* l_RocketSpark = m_InstanceScript->instance->GetCreature(m_InstanceScript->GetData64(GrimrailDepotData::DataRocketspark)))
                {
                    if (Creature* l_Borka = m_InstanceScript->instance->GetCreature(m_InstanceScript->GetData64(GrimrailDepotData::DataBorka)))
                    {
...
static void StartRocketSparkAndBorka(InstanceScript* p_Instance, Creature* p_Me, uint64 p_TargetGuid)
{
...
public:
    ...
    struct boss_rocketsparkAI : public BossAI
    {
        ...
        void HandleEncounterStartDoor()
        {
            if (m_Instance != nullptr)
            {
                if (GameObject* l_EncounterStartDoor = m_Instance->instance->GetGameObject(m_Instance->GetData64(GrimrailDepotData::DataSpikedGateBoss1)))
                {
                    /// Activate
        ...
        void HandleEncounterFinishDoor()
        {
            if (m_Instance != nullptr)
            {
                if (GameObject* l_EncounterFinishDoor = m_Instance->instance->GetGameObject(m_Instance->GetData64(GrimrailDepotData::DataIronWroughtGate)))
                {
                    /// Activate
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            ...
            case eRocksparkAndBorkaMovementInformed::MovementHigherPlatform:
                ...
                me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_DISABLE_MOVE);
                break;
            default:
                break;
            }
        ...
        void DoAction(int32 const p_Action) override
        {
            ...
            case eRocksparkAndBorkaActions::ActionRocketSparkShootMissilesVX18:
                for (uint8 l_I = 0; l_I < 6; l_I++)
                {
                    ...
                    if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_RANDOM, 0, 100.0f, true))
                        l_Target->GetRandomNearPosition(l_Position, 20.0f);
            ...
            case eRocksparkAndBorkaActions::ActionRocketSparkNewPlan:
                if (!m_NewPlan)
                {
                    ...
                    me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
            ...
            case eRocksparkAndBorkaActions::ActionMadDashContact:
                ...
                me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_DISABLE_MOVE | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
                break;
            default:
                break;
            }
...
public:
    ...
    struct grimrail_depot_rocketspark_mob_borkaAI : public BossAI
    {
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            ...
            {
                float l_X = me->m_positionX + 50 * cos(me->m_orientation);
                float l_Y = me->m_positionY + 50 * sin(me->m_orientation);
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
                ...
                if (m_MadDashTargetGUID)
                {
                    if (m_Hit)
                    {
                        ...
                        if (m_MadDashTargetFinderDiff <= p_Diff)
                        {
                            ...
                            me->GetPlayerListInGrid(l_ListPlayer, 1.5f, true);
                        ...
                        if (Creature* l_RocketSpark = m_Instance->instance->GetCreature(m_Instance->GetData64(GrimrailDepotData::DataRocketspark)))
                        {
                            if (Creature* l_Target = Creature::GetCreature(*me, m_MadDashTargetGUID))
                            {
            ...
            case eBorkaEvents::EventMadDash: /// 01
            {
                if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_RANDOM, 0, 100.0f, true))
                {
                    ...
                    if (m_MadDashTargetGUID)
                    {
                        if (Creature* l_Creature = Creature::GetCreature(*me, m_MadDashTargetGUID))
    ...
    class grimrail_depot_rocketspark_spell_x21_01a_missile_barrage_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* p_AurEff)
        {
            ...
            if (GetCaster())
            {
                ...
                GetCaster()->GetPlayerListInGrid(l_ListPlayers, 100.0f, true);
                if (!l_ListPlayers.empty())
                {
                    for (Player* l_Itr : l_ListPlayers)
                    {
                        ...
                        /// Cosmetic
                        Position l_Position;
                        l_Itr->GetRandomNearPosition(l_Position, 10.0f);
