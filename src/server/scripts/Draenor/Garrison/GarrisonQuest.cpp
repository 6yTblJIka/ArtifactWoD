...
namespace MS {
	namespace Garrison
	{
		...
		void GarrisonQuestPlayerScript::OnItemLooted(Player* p_Player, Item* p_Item)
		{
			...
			if (p_Player->HasQuest(Quests::Alliance_BuildYourBarracks) && p_Item && p_Item->GetEntry() == Items::ITEM_GARRISON_BLUEPRINT_BARRACKS_LEVEL1)
			{
				p_Player->QuestObjectiveSatisfy(39422, 1, QUEST_OBJECTIVE_TYPE_CRITERIA_TREE);
				return;
			}
		...
		void spell_learning_blueprint::spell_learning_blueprint_SpellScript::HandleAfterCast()
		{
			if (GetCaster() && GetCaster()->ToPlayer())
			{
				...
				{
					if (l_Player->GetGarrison() && l_Player->HasQuest(Quests::Alliance_BuildYourBarracks))
						l_Player->QuestObjectiveSatisfy(36163, 1, QUEST_OBJECTIVE_TYPE_CRITERIA_TREE, l_Player->GetGUID());
				}
			}
		}
