        ...
        if (Player* l_Player = GetCaster()->ToPlayer())
        {
            if (MS::Garrison::Manager* l_GarrisonMgr = l_Player->GetGarrison())
            {
                switch (l_GarrisonMgr->GetGarrisonLevel())
                {
                   ...
                   case 3: ///< Garrison level 3
				   {
					   if (l_Player->GetTeamId() == TEAM_ALLIANCE)
						   l_Player->TeleportTo(1159, 1938.7103f, 338.6705f, 90.2805f, 5.2780f); ///< Alliance Garrison Lvl 3
					   else
						   l_Player->TeleportTo(1153, 5562.7666f, 4600.6103f, 141.7110f, 2.2361f); ///< Horde Garrison Lvl 3
				   }
                   default:
                        break;
                }
