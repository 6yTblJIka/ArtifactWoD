...
void WorldSession::HandleSetFactionOpcode(WorldPacket& recvPacket)
{
    ...
    if (choice == JOIN_THE_HORDE)
    {
		...
		if (const Quest* l_Quest = sObjectMgr->GetQuestTemplate(31012))
		{
    ...
    else if (choice == JOIN_THE_ALLIANCE)
    {
        ...
        m_Player->learnSpell(108127, false); // Language Pandaren
		if (const Quest* l_Quest = sObjectMgr->GetQuestTemplate(30987))
		{
			m_Player->AddQuest(l_Quest, m_Player);
		}
    }
...
void WorldSession::HandleSaveCUFProfiles(WorldPacket& p_RecvPacket)
{
    ...
    std::vector<CUF::JamCliCUFProfile*> l_Profiles;

    l_Profiles.reserve(l_ProfileCount);

    for (uint32 l_I = 0; l_I < l_ProfileCount; ++l_I)
    {
        CUF::JamCliCUFProfile* l_Profile = new CUF::JamCliCUFProfile();

        l_Profile->Name.resize(p_RecvPacket.ReadBits(7));
        l_Profile->KeepGroupsTogether            = p_RecvPacket.ReadBit();
        l_Profile->DisplayPets                   = p_RecvPacket.ReadBit();
        l_Profile->DisplayMainTankAndAssist      = p_RecvPacket.ReadBit();
        l_Profile->DisplayHealPrediction         = p_RecvPacket.ReadBit();
        l_Profile->DisplayAggroHighlight         = p_RecvPacket.ReadBit();
        l_Profile->DisplayOnlyDispellableDebuffs = p_RecvPacket.ReadBit();
        l_Profile->DisplayPowerBar               = p_RecvPacket.ReadBit();
        l_Profile->DisplayBorder                 = p_RecvPacket.ReadBit();
        l_Profile->UseClassColors                = p_RecvPacket.ReadBit();
        l_Profile->HorizontalGroups              = p_RecvPacket.ReadBit();
        l_Profile->DisplayNonBossDebuffs         = p_RecvPacket.ReadBit();
        l_Profile->DynamicPosition               = p_RecvPacket.ReadBit();
        l_Profile->Locked                        = p_RecvPacket.ReadBit();
        l_Profile->Shown                         = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate2Players          = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate3Players          = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate5Players          = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate10Players         = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate15Players         = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate25Players         = p_RecvPacket.ReadBit();
        l_Profile->AutoActivate40Players         = p_RecvPacket.ReadBit();
        l_Profile->AutoActivateSpec1             = p_RecvPacket.ReadBit();
        l_Profile->AutoActivateSpec2             = p_RecvPacket.ReadBit();
        l_Profile->AutoActivatePvP               = p_RecvPacket.ReadBit();
        l_Profile->AutoActivatePvE               = p_RecvPacket.ReadBit();

        p_RecvPacket >> l_Profile->FrameHeight;
        p_RecvPacket >> l_Profile->FrameWidth;
        p_RecvPacket >> l_Profile->SortBy;
        p_RecvPacket >> l_Profile->HealthText;
        p_RecvPacket >> l_Profile->TopPoint;
        p_RecvPacket >> l_Profile->BottomPoint;
        p_RecvPacket >> l_Profile->LeftPoint;
        p_RecvPacket >> l_Profile->TopOffset;
        p_RecvPacket >> l_Profile->BottomOffset;
        p_RecvPacket >> l_Profile->LeftOffset;

        l_Profile->Name = p_RecvPacket.ReadString(l_Profile->Name.size());

        l_Profiles.emplace_back(l_Profile);
    }

    GetPlayer()->ReplaceCUFProfiles(l_Profiles, false);
}
