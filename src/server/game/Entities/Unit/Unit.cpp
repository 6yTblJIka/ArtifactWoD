...
bool GlobalCooldownMgr::HasGlobalCooldown(SpellInfo const* spellInfo) const
{
	...
	uint32 val = getMSTimeDiff(itr->second.cast_time, getMSTime());
    return itr != m_GlobalCooldowns.end() && itr->second.duration && val < itr->second.duration;
}

uint32 GlobalCooldownMgr::TimeLeftOnGCD(SpellInfo const* spellInfo)
{
...
void Unit::GetConversationListInRange(std::list<Conversation*>& p_List, float p_Range) const
{
    CellCoord l_Coords(JadeCore::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell l_Cell(l_Coords);
    l_Cell.SetNoCreate();
