...
void AuraApplication::BuildUpdatePacket(ByteBuffer & p_Data, bool p_Remove, uint32 p_OverrideSpellID) const
{
    ...
    p_Data << uint32(l_Base->GetSpellInfo()->GetSpellXSpellVisualId(GetTarget()));                              ///< SpellVisuals
...
m_castItemGuid(castItem ? castItem->GetGUID() : 0), m_applyTime(time(NULL)), m_applyTimeMS(getMSTime()),
